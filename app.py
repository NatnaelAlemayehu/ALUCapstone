...
def extract_keypoints(results):
    ...
    global hand_status, nolefthand, norighthand
    nolefthand = False
    norighthand = False
    hand_status = True    
    ...
    if (nolefthand) and (norighthand):
        hand_status = False
    
    print("handstatus is {}".format(hand_status))   

    return hand_status, np.concatenate([pose, face, lh, rh])

actions = np.array(['ጤና ይስጥልኝ','እግዚአብሔር','እናት','አባት','የኔ','አንድ','ሁለት','ሶስት','አራት'])
outputimage = ""
...
def generate_frames():  
    ...
    global predictedwords
    predictedwords = ""

    camera=cv2.VideoCapture(0)

    model = keras.models.load_model("./righthandsignmodel.h5")
    sequence = []
    sentence = []            
    predictions = []
    threshold = 0.8
    ...
    with mp_holistic.Holistic(min_detection_confidence=0.5, min_tracking_confidence=0.5) as holistic:
        while camera.isOpened():
            ...
            # 2. Prediction logic
            handstat, keypoints = extract_keypoints(results)
            sequence.append(keypoints)
            sequence = sequence[-30:]
            
            if len(sequence) == 30 and handstat:
